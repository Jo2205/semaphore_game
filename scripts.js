...
const CONFIG = {
  API_URL: "https://jo2254-gabut.hf.space/predict", // Flask backend endpoint
...
async function enableCamera(mode) {
  ...
  try {
    ...
    // Hide placeholder
    const placeholder = videoElement.parentElement.querySelector(
      ".camera-placeholder"
    );
    if (placeholder) {
...
async function detectSemaphore() {
  ...
  try {
    ...
    console.log("📷 Base64 sample:", imageData.slice(0, 100));
  ...
  detectionInterval = setInterval(async () => {
    ...
    if (detection) {
      ...
      if (detection.letter === gameState.currentLetter) {
        showNotification(
          `Great! You showed "${detection.letter}" correctly! (${Math.round(
        ...
        showNotification(
          `Detected "${detection.letter}" but target is "${gameState.currentLetter}"`,
          "info"
        );
      }
    }
...
function startSinglePlayerGame() {
  ...
  showNotification(
    "Game started! Show the target letter with semaphore",
    "success"
  );
}
...
function handleGameDetection(detection) {
  const currentPlayer = gameState.isMultiplayer
    ? gameState.players[gameState.currentPlayer]
    : "Player";

  if (detection.letter === gameState.targetLetter) {
    // Correct answer!
    gameState.score[currentPlayer] =
      (gameState.score[currentPlayer] || 0) + CONFIG.SCORE_PER_CORRECT;

    // Removed notification for single player mode as per user request
    if (gameState.isMultiplayer) {
      showNotification(
        `Correct! +${CONFIG.SCORE_PER_CORRECT} points (${Math.round(
          detection.confidence * 100
        )}%)`,
        "success"
      );
    ...
    if (gameState.isMultiplayer) {
      showNotification(
        `Detected "${detection.letter}" but target is "${gameState.targetLetter}"`,
        "info"
      );
    }
  }
...
function updateGameUI() {
  try {
    ...
    let scoreElement = document.getElementById(
      gameState.isMultiplayer ? "mp-score" : "game-score"
    );
    if (scoreElement) {
      const currentPlayer = gameState.isMultiplayer
        ? gameState.players[gameState.currentPlayer]
      ...
      console.log(
        `Score updated for ${currentPlayer}: ${
          gameState.score[currentPlayer] || 0
        }`
      );
    } else {
    ...
    let timerElement = document.getElementById(
      gameState.isMultiplayer ? "mp-timer" : "game-timer"
    );
    if (timerElement) {
...
function startGameTimer() {
  ...
  gameState.timer = gameState.isMultiplayer
    ? CONFIG.MULTIPLAYER_TIME
    : CONFIG.SINGLE_GAME_TIME;
...
function startPlayerTurn() {
  ...
  showNotification(
    `${currentPlayerName} - Start showing semaphore letters!`,
    "success"
  );
}
...
function startMultiplayerGame() {
  ...
  if (!gameState.isMultiplayer || gameState.players.length === 0) {
    showNotification(
      "No players found. Please setup multiplayer first.",
      "error"
    );
    return;
  }
...
function handleMultiplayerTurnEnd() {
  ...
  showNotification(
    `${currentPlayerName} finished with ${currentScore} points!`,
    "info"
  );
...
function loadGameHistory() {
  ...
  gameState.gameHistory.forEach((session, index) => {
    ...
    const playersHtml = session.players
      .map(
        (player, pIndex) => `
...
function showInstructions() {
  ...
  const currentLetter = gameState.isPracticeMode
  ...
  if (modalInstruction)
    modalInstruction.textContent = SEMAPHORE_DATA[currentLetter];
...
async function testAPIConnection() {
  try {
    ...
    showNotification(
      "Cannot connect to AI service. Please check if Flask server is running.",
      "error"
    );
    return false;
  }
...
function handleComboSystem(isCorrect) {
  ...
  if (isCorrect) {
    if (currentTime - lastCorrectTime < 5000) {
      // Dalam 5 detik
      comboCount++;
      if (comboCount >= 3) {
        ...
        showNotification(
          `🔥 COMBO x${comboCount}! Bonus +${bonusPoints} points!`,
          "success"
        );
      }
    } else {
...
const achievements = {
  ...
  speed_demon: {
    name: "Speed Demon",
    description: "Get 5 correct answers in 30 seconds",
  },
  perfect_game: {
    name: "Perfect Game",
